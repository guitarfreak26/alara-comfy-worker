...
REGISTRY_PATH = Path(os.getenv("WORKFLOW_REGISTRY", APP_DIR / "workflow_registry.yaml"))
COMFY_START_TIMEOUT = int(os.getenv("COMFY_START_TIMEOUT", "300"))
PROMPT_TIMEOUT = int(os.getenv("PROMPT_TIMEOUT", "900"))
POLL_INTERVAL = float(os.getenv("POLL_INTERVAL", "1.0"))
MAX_COUNT = int(os.getenv("MAX_COUNT", "4"))
...
def _load_registry() -> dict[str, Any]:
    ...
    if _registry_cache is None:
        _registry_cache = yaml.safe_load(REGISTRY_PATH.read_text()) or {}
...
def _patch_workflow(workflow: dict[str, Any], workflow_config: dict[str, Any], payload: dict[str, Any], job_id: str) -> tuple[dict[str, Any], int]:
    ...
    _set_path(patched, patch_points["prompt"], prompt)
    _set_path(patched, patch_points["negative_prompt"], negative_prompt)
    _set_path(patched, patch_points["seed"], seed)
    _set_path(patched, patch_points["width"], width)
    _set_path(patched, patch_points["height"], height)
    _set_path(patched, patch_points["count"], count)

    if "filename_prefix" in patch_points:
    ...
    for node_id, node_output in history_outputs.items():
        ...
        for image in node_output.get("images", []) or []:
            item = {
                ...
                "path": str(COMFY_DIR / "output" / image.get("subfolder", "") / image.get("filename", "")),
            }
            if include_base64:
...
def handler(job: dict[str, Any]) -> dict[str, Any]:
    ...
    workflow_id = str(job_input.get("workflow_id") or "seoyeon-zimage-full-quality-v1")
    include_base64 = bool(job_input.get("return_base64", True))
